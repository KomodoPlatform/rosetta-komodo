...
// monetary amount.
const (
	AmountMegaKMD  AmountUnit = 6
	AmountKiloKMD  AmountUnit = 3
	AmountKMD      AmountUnit = 0
	AmountMilliKMD AmountUnit = -3
	AmountMicroKMD AmountUnit = -6
	AmountKMDtoshi AmountUnit = -8
)

// String returns the unit as a string.  For recognized units, the SI
// prefix is used, or "Kmdtoshi" for the base unit.  For all unrecognized
// units, "1eN KMD" is returned, where N is the AmountUnit.
func (u AmountUnit) String() string {
	...
	case AmountKMDtoshi:
		return "Kmdtoshi"
	default:
		return "1e" + strconv.FormatInt(int64(u), 10) + " KMD"
	}
}
...
//
// NewAmount is for specifically for converting KMD to Kmdtoshi.
// For creating a new Amount with an int64 value which denotes a quantity of Satoshi,
// do a simple type conversion from type int64 to Amount.
// See GoDoc for example: http://godoc.org/github.com/DeckerSU/rosetta-komodo/btcutil#example-Amount
func NewAmount(f float64) (Amount, error) {
	...
	case math.IsInf(f, 1):
		fallthrough
	case math.IsInf(f, -1):
		return 0, errors.New("invalid komodo amount")
	}

	return round(f * KmdtoshiPerKmd), nil
}
...
// ToKMD is the equivalent of calling ToUnit with AmountKMD.
func (a Amount) ToKMD() float64 {
	return a.ToUnit(AmountKMD)
}

// Format formats a monetary amount counted in bitcoin base units as a
// string for a given unit.  The conversion will succeed for any unit,
// however, known units will be formated with an appended label describing
// the units with SI notation, or "Kmdtoshi" for the base unit.
func (a Amount) Format(u AmountUnit) string {
	units := " " + u.String()
	return strconv.FormatFloat(a.ToUnit(u), 'f', -int(u+8), 64) + units
}

// String is the equivalent of calling Format with AmountKMD.
func (a Amount) String() string {
	return a.Format(AmountKMD)
}
