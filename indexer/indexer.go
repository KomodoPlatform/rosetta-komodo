...
import (
	...
	"github.com/DeckerSU/rosetta-komodo/services"
...
type Client interface {
	...
	GetRawBlock(context.Context, *types.PartialBlockIdentifier) (*komodo.Block, []string, error)
	ParseBlock(
		context.Context,
		*komodo.Block,
...
func Initialize(
	...
	asserter, err := asserter.NewClientWithOptions(
		...
		komodo.OperationTypes,
		komodo.OperationStatuses,
		services.Errors,
		nil,
...
// waitForNode returns once komodod is ready to serve
// block queries.
func (i *Indexer) waitForNode(ctx context.Context) error {
	...
	for {
		...
		logger.Infow("waiting for komodod...")
...
// Sync attempts to index Komodo blocks using
...
// Prune attempts to prune blocks in komodod every
// pruneFrequency.
func (i *Indexer) Prune(ctx context.Context) error {
	...
	for {
		...
		case <-tc.C:
			...
			logger.Infow("attempting to prune komodod", "prune height", pruneHeight)
			prunedHeight, err := i.client.PruneBlockchain(ctx, pruneHeight)
			if err != nil {
				logger.Warnw(
					"unable to prune komodod",
					"prune height", pruneHeight,
					"error", err,
				)
			} else {
				logger.Infow("pruned komodod", "prune height", prunedHeight)
			}
		}
	}
}
...
func (i *Indexer) findCoin(
	...
	for ctx.Err() == nil {
		...
		// multiple listeners)
		transactionHash := komodo.TransactionHash(coinIdentifier)
...
func (i *Indexer) findCoins(
	...
	for _, coinIdentifier := range remainingCoins {
		// Wait on Channel
		txHash := komodo.TransactionHash(coinIdentifier)
...
func (i *Indexer) GetScriptPubKeys(
	ctx context.Context,
	coins []*types.Coin,
) ([]*komodo.ScriptPubKey, error) {
	databaseTransaction := i.database.ReadTransaction(ctx)
	defer databaseTransaction.Discard(ctx)

	scripts := make([]*komodo.ScriptPubKey, len(coins))
	for j, coin := range coins {
		coinIdentifier := coin.CoinIdentifier
		transactionHash, networkIndex, err := komodo.ParseCoinIdentifier(coinIdentifier)
		if err != nil {
		...
		for _, op := range transaction.Operations {
			if op.Type != komodo.OutputOpType {
				continue
			}
			...
			var opMetadata komodo.OperationMetadata
