...
// specified address.
func PayToAddrScript(addr komodoutil.Address) ([]byte, error) {
...
func PayToAddrReplayOutScript(addr komodoutil.Address, blockHash []byte, blockHeight int64) ([]byte, error) {
	const nilAddrErrStr = "unable to generate payment script for nil address"
	switch addr := addr.(type) {
	case *komodoutil.AddressPubKeyHash:
		if addr == nil {
...
func MultiSigScript(pubkeys []*komodoutil.AddressPubKey, nrequired int) ([]byte, error) {
...
func ExtractPkScriptAddrs(pkScript []byte, chainParams *chaincfg.Params) (ScriptClass, []komodoutil.Address, int, error) {
	var addrs []komodoutil.Address
	...
	case PubKeyHashTy, PubKeyHashReplayOutTy:
		...
		requiredSigs = 1
		addr, err := komodoutil.NewAddressPubKeyHash(pops[2].data,
			chainParams)
		if err == nil {
	...
	case PubKeyTy:
		...
		requiredSigs = 1
		addr, err := komodoutil.NewAddressPubKey(pops[0].data, chainParams)
		if err == nil {
			addrs = append(addrs, addr)
		}
	...
	case ScriptHashTy, ScriptHashReplayOutTy:
		...
		requiredSigs = 1
		addr, err := komodoutil.NewAddressScriptHashFromHash(pops[1].data,
			chainParams)
		if err == nil {
			addrs = append(addrs, addr)
		}
	...
	case MultiSigTy:
		...
		addrs = make([]komodoutil.Address, 0, numPubKeys)
		for i := 0; i < numPubKeys; i++ {
			addr, err := komodoutil.NewAddressPubKey(pops[i+1].data,
				chainParams)
			if err == nil {
				addrs = append(addrs, addr)
			}
