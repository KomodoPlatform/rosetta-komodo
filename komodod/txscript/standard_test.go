...
func newAddressPubKey(serializedPubKey []byte) komodoutil.Address {
	addr, err := komodoutil.NewAddressPubKey(serializedPubKey,
...
// test source code.
func newAddressPubKeyHash(pkHash []byte) komodoutil.Address {
	addr, err := komodoutil.NewAddressPubKeyHash(pkHash, &chaincfg.MainNetParams)
	if err != nil {
...
// as a helper since the only way it can fail is if there is an error in the
// test source code.
func newAddressScriptHash(scriptHash []byte) komodoutil.Address {
	addr, err := komodoutil.NewAddressScriptHashFromHash(scriptHash,
		&chaincfg.MainNetParams)
	if err != nil {
...
func TestPayToAddrScript(t *testing.T) {
	...
	p2pkhMain, err := komodoutil.NewAddressPubKeyHash(hexToBytes("e34cce70c86"+
	...
	p2shMain, _ := komodoutil.NewAddressScriptHashFromHash(hexToBytes("e8c300"+
	...
	p2pkCompressedMain, err := komodoutil.NewAddressPubKey(hexToBytes("02192d"+
	...
	p2pkCompressed2Main, err := komodoutil.NewAddressPubKey(hexToBytes("03b0b"+
	...
	p2pkUncompressedMain, err := komodoutil.NewAddressPubKey(hexToBytes("0411"+
	...
	tests := []struct {
		...
		{(*komodoutil.AddressPubKeyHash)(nil), "", errUnsupportedAddress},
		{(*komodoutil.AddressScriptHash)(nil), "", errUnsupportedAddress},
		{(*komodoutil.AddressPubKey)(nil), "", errUnsupportedAddress},
...
func TestMultiSigScript(t *testing.T) {
	...
	tests := []struct {
		keys      []*komodoutil.AddressPubKey
		nrequired int
		expected  string
		err       error
	}{
		{
			[]*komodoutil.AddressPubKey{
				p2pkCompressedMain,
				p2pkCompressed2Main,
			},
			1,
