...
import (
	...
	"github.com/DeckerSU/rosetta-komodo/indexer"
...
func startOnlineDependencies(
	...
	g *errgroup.Group,
) (*komodo.Client, *indexer.Indexer, error) {
	client := komodo.NewClient(
		komodo.LocalhostURL(cfg.RPCPort),
		cfg.GenesisBlockIdentifier,
		cfg.Currency,
	)

	g.Go(func() error {
		return komodo.StartKOMODOD(ctx, cfg.ConfigPath, g)
	})
	...
	//komodod does not support manual pruning
	//g.Go(func() error {
	//	return i.Prune(ctx)
	//})
...
func main() {
	...
	if signalReceived {
		logger.Fatalw("rosetta-komodo halted")
	}

	if err != nil {
		logger.Fatalw("rosetta-komodo sync failed", "error", err)
	}
}
